...
public class MyFriendsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        longclicked = new myadapter.longclicked() {
            ...
            public void onLongclick(final int position) {
                ...
                final AlertDialog.Builder builder = new AlertDialog.Builder(MyFriendsActivity.this);
                builder.setTitle("Remove Friend from your chat list")
                        .setMessage("Do you really want to remove " + actual_list.get(position).getName())
                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                deleteuser(position,dialog);
                            }
        ...
        myadapter = new myadapter(MyFriendsActivity.this, actual_list, lastmessages, userClicked, longclicked);
    ...
    private void deleteuser(final int position, final DialogInterface dilog) {
        FirebaseDatabase.getInstance().getReference("Friends/"+auth.getCurrentUser().getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot snapshot1:snapshot.getChildren()){
                    if (snapshot1.getValue().toString().equals(actual_list.get(position).getUid())){
                        FirebaseDatabase.getInstance().getReference("Friends/"+auth.getCurrentUser().getUid()+"/"+snapshot1.getKey()).removeValue();
                        break;
                    }
                }
                dilog.dismiss();
